...
'''
Yuz Tespiti1.1 'e yapilna guncellemeler:

1-)daha guclu bir modele gecis yapildi
2-)yuzun etrafina cizilen kareye confidence yuzdesinin yazimi
3-)confidence e gore yuz sayimi
4-)confidence e gore yuzun etrafina cizilen karenin rengi dinamik hale getirildi
5-)goruntulerin sol ust kosesine anlik fps yazildi
6-)temaya gore degisen butonun renkleri degistirildi
7-)kucuk performans iyilestirmeleri
...
class Config:
    ...
    net = cv.dnn.readNetFromCaffe(DNN_PROTOTXT , DNN_MODEL)
...
class CustomWidget(QFrame):
    def __init__(self):
        ...
        pixmap = QPixmap("anlikyuztespiti/photos/CBN-2.png").scaled(40, 40, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)
